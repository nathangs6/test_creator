...
export function NewSubCollectionForm({ setOpen, collectionID, collectionName }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await API.post("/subcollection/new/"+collectionID, {newSubCollectionName: name});
...
export function RenameSubCollectionForm({ setOpen, subCollectionData }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const response = await API.post("/subcollection/rename/"+id, {newSubCollectionName: name});
...
export function DeleteSubCollectionForm({ setOpen, subCollectionData }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        await API.post("/subcollection/delete/"+subCollectionData.id, {});
